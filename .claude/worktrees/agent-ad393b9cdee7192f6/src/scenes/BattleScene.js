...
export default class BattleScene extends Phaser.Scene {
  ...
  _showWebView() {
    ...
    combatDice.forEach(eDie => {
      ...
      const value = Math.floor(eDie.data.currentFaceIdx / 2) + 1;
  ...
  _enemyRollPhase() {
    ...
    // Throw enemy combat dice staggered
    enemyDiceConfigs.forEach((cfg, i) => {
      this.time.delayedCall(i * 220, () => {
        ...
        const angle = Phaser.Math.FloatBetween(-Math.PI * 0.65, Math.PI * 0.65);
        const speed = Phaser.Math.FloatBetween(5, 11);
        const sx    = this.enemyPos.x + Phaser.Math.FloatBetween(-20, 20);
        const sy    = this.enemyPos.y + ENEMY_BUMPER_R + 6;
        const eDie  = this._spawnDie(data, sx, sy, Math.cos(angle) * speed, Math.abs(Math.sin(angle)) * speed, false, -1);
        eDie._rolling          = true;
    ...
    // Pop obstacle dice in, staggered after combat dice
    const count   = this.enemyDef.obstacleCount ?? 0;
    const MARGIN  = 36;
    ...
    positions.forEach((pos, i) => {
      const delay = enemyDiceConfigs.length * 220 + i * 140;
      this.time.delayedCall(delay, () => {
        if (this.phase === 99) return;
        const eDie = this._spawnDie({ isObstacle: true, currentFaceIdx: 0, sides: 6 }, pos.x, pos.y, 0, 0, false, -1);
        eDie._finalFaceIdx = 0;
        eDie._rolling = false;
        eDie.img.setAlpha(0).setScale(0.1);
        eDie.lbl.setAlpha(0);
        eDie.valLbl.setAlpha(0);
        this.tweens.add({ targets: eDie.img, alpha: 1, scaleX: 1, scaleY: 1, duration: 250, ease: 'Back.Out' });
        this.tweens.add({ targets: [eDie.lbl, eDie.valLbl], alpha: 1, duration: 200, delay: 100 });
      });
    });

    const minDelay = enemyDiceConfigs.length * 220 + count * 140 + 700;
  ...
  _rerollDie(dieRef) {
    ...
    if (dieRef.isPlayer) {
      ...
      const maxIdx = data.faces ? data.faces.length - 1 : (data.sides ?? 6) - 1;
      data.currentFaceIdx = Phaser.Math.Between(0, maxIdx);
    }
    ...
    this.allDice.forEach(d => {
      ...
      if (motion < SETTLE_VEL) {
        ...
        if (d.isPlayer) {
          ...
          // New enemy combat die — lock in final value display
          d.lbl.setText(ENEMY_DIE_TYPE_SYMS[d.data.type] ?? '?');
          d.lbl.setColor(ENEMY_DIE_TYPE_COLORS[d.data.type] ?? '#ffffff');
          d.valLbl.setText(String(Math.floor(d._finalFaceIdx / 2) + 1));
        }
        ...
        if (now - d._lastCycleMs > interval) {
          ...
          if (d.isPlayer) {
            ...
            if (d.data.type && !d.data.isObstacle) {
              // New enemy combat die — cycle the value, keep type sym fixed
  ...
  _showTutPanel(step) {
    ...
    const STEPS = [
      ...
      {
        title: 'Enemy Dice',
        body:  "The enemy has thrown their own dice onto the table! Their dice determine what they'll do at turn end. Tap the enemy circle to see which dice belong to them.",
        y:     490,
      },
      {
        title: 'Throwing Dice',
