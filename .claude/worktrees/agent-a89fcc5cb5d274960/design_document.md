...
The bottom section of the inspector shows any upgrades the die carries as a flat list: `● UpgradeName` in the upgrade's theme color. Upgrades are stored as `upgradeState.takenUpgrades[]` (array of upgrade IDs).
...
After each battle victory the player enters the Upgrade Screen. Three upgrade cards are presented, then the player picks a die to receive the chosen upgrade.

### 10.1 Flow

1. **Card Step** — three upgrade cards are drawn from the full 37-upgrade pool via the Lottery System. Each card shows the upgrade's rarity tag (COMMON / UNCOMMON / RARE) and its full description.
2. **Die Picker** — after tapping a card, the player chooses which die receives the upgrade. Any die that already has that upgrade is grayed out and unselectable.
3. **Apply** — the upgrade ID is pushed into `dc.upgradeState.takenUpgrades[]`. A **Heal** footer button is available at the Card Step as an alternative (skips the upgrade, heals 5 HP).

### 10.2 Draw Logic

Upgrades are drawn via `LotterySystem(UPGRADES, [])`. Ticket weights by rarity:
...
Synergy bonuses from owned IDs are not applied at draw time in the current version.

### 10.3 Upgrade List

All 37 upgrades share a flat structure: `{ id, name, rarity, color, planned, synergies, description }`. Upgrades marked **[PLANNED]** are defined in data but their battle effects are not yet implemented.

| # | id | name | rarity | planned |
|---|-----|------|--------|---------|
| 1 | reduction | Reduction | common | no |
| 2 | more_reduction | More Reduction | uncommon | no |
| 3 | status_damage | Status Damage | uncommon | no |
| 4 | blocking_stance | Blocking Stance | rare | no |
| 5 | buff | Buff | common | no |
| 6 | buff_reach | Buff Reach | uncommon | no |
| 7 | buffet | Buffet | rare | no |
| 8 | wall_ball | Wall Ball | common | no |
| 9 | featherlight | Featherlight | uncommon | no |
| 10 | wall_radiation | Wall Radiation | uncommon | no |
| 11 | mine | Mine | uncommon | **yes** |
| 12 | poison | Poison | common | no |
| 13 | cultist | Cultist | uncommon | no |
| 14 | slow_drip | Slow Drip | uncommon | no |
| 15 | glass | Glass | common | no |
| 16 | bulletproof_glass | Bulletproof Glass | uncommon | no |
| 17 | destroy_contacts | Destroy Contacts | rare | no |
| 18 | gravity_well | Gravity Well | uncommon | no |
| 19 | black_hole | Black Hole | rare | no |
| 20 | sign | Sign | uncommon | **yes** |
| 21 | ventriloquist | Ventriloquist | uncommon | no |
| 22 | bumper | Bumper | common | no |
| 23 | explosive_contact | Explosive Contact | uncommon | **yes** |
| 24 | trigger_materials | Trigger Materials | uncommon | **yes** |
| 25 | shiv | Shiv | common | no |
| 26 | shiv_trigger | Shiv Trigger | uncommon | no |
| 27 | shiv_tier_plus | Shiv Tier+ | uncommon | no |
| 28 | shiv_quantity | Shiv Quantity | uncommon | no |
| 29 | poison_burst | Poison Burst | uncommon | no |
| 30 | specialist_swarm | Specialist Swarm | rare | **yes** |
| 31 | heavyweight | Heavyweight | uncommon | no |
| 32 | greased | Greased | uncommon | no |
| 33 | phantom_step | Phantom Step | rare | **yes** |
| 34 | magnetic | Magnetic | uncommon | no |
| 35 | vanguard | Vanguard | uncommon | no |
| 36 | finisher | Finisher | uncommon | no |
| 37 | consume | Consume | rare | no |

### 10.4 Heal Option

A flat heal of **5 HP** is available as the footer button on the upgrade screen. Choosing it skips the upgrade for that battle.

### 10.5 Between-Battle Flow
