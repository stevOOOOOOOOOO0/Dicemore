...
export default class BattleScene extends Phaser.Scene {
  ...
  _setupCollisions() {
    this.matter.world.on('collisionstart', (event) => {
      ...
      event.pairs.forEach(({ bodyA, bodyB }) => {
        ...
        if (dA && dB) {
          // Glass upgrade: shatters on first die contact
          const glassA = dA.isPlayer && (dA.data.material === 'glass' || this._hasDieUpgrade(dA.data, 'glass'));
          const glassB = dB.isPlayer && (dB.data.material === 'glass' || this._hasDieUpgrade(dB.data, 'glass'));
          ...
          // Destroy Contacts upgrade: dice this die contacts become Cracked
          const _checkDestroyContacts = (attacker, target) => {
            if (!attacker.isPlayer) return;
            if (!this._hasDieUpgrade(attacker.data, 'destroy_contacts')) return;
            if (!target.img?.active) return;
            target._contactHits = (target._contactHits ?? 0) + 1;
            if (target._contactHits === 1) {
              target._cracked = true;
              this._floatText(target.img?.x ?? 0, (target.img?.y ?? 0) - 24, 'CRACKED', '#ff6633');
            } else if (target._contactHits >= 3) {
              this._floatText(target.img?.x ?? 0, (target.img?.y ?? 0) - 24, 'SHATTER!', '#ff6633');
              this._applyDieFaceImmediate(target, true, null, null, true);
              this._applyDieFaceImmediate(target, true, null, null, true);
              this._shatterDie(target);
            }
          };
          _checkDestroyContacts(dA, dB);
          _checkDestroyContacts(dB, dA);

          // Black Hole: any player die that contacts it during pull is destroyed
        ...
        // Mine contact
        if (other?.label === 'mine') {
          const mine = this._mines.find(m => m.body === other && !m.triggered);
        ...
        if (dieRef && other.label === 'enemyBumper') {
          ...
          // Base chip damage (1) + Bumper upgrade bonus (+1)
          const bumperBonus = (dieRef.isPlayer && this._hasDieUpgrade(dieRef.data, 'bumper')) ? 1 : 0;
  ...
  _setupPointer() {
    ...
    this.input.on('pointerup', (ptr) => {
      ...
      if (this.aimActive && this.phase === PHASE.PLAYER_ROLL) {
        ...
        this._nextThrowSpeedBonus = 0;
        const spd = Math.min(MAX_THROW_SPEED, Math.max(7, len * 0.14) + speedBonus);
  ...
  _enemyRollPhase() {
    ...
    positions.forEach((pos, i) => {
      this.time.delayedCall(i * 140, () => {
        const eDie = this._spawnDie({ isObstacle: true, currentFaceIdx: 0, sides: 6 },
  ...
  _commitPhase() {
    ...
    this.time.delayedCall(600, () => {
      ...
      subs.forEach(sub => {
        switch (sub.type) {
          case 'attack': {
            let dmg = Math.max(0, (sub.value + this.enemyStrength) - (this._reductionStacks ?? 0));
            if (this.vulnerable > 0) dmg = Math.ceil(dmg * 1.5);
            if (this.enemyWeakened)  dmg = Math.floor(dmg * 0.5);
          ...
          case 'vulnerable':
            ...
            this.bus.emit('ON_STATUS_APPLIED', { target: 'player', status: 'vulnerable', amount: 3 });
            break;
          case 'frail':
            ...
            this.bus.emit('ON_STATUS_APPLIED', { target: 'player', status: 'frail', amount: 3 });
            break;
        }
      });
  ...
  _applyDieFaceImmediate(dieRef, skipRune = false, snapX = null, snapY = null, _skipUpgrades = false) {
    ...
    // Poison upgrade: convert die effect to poison stacks
    if (!_skipUpgrades && this._hasDieUpgrade(data, 'poison')) {
      ...
      this._applyFlatUpgradeEffects(dieRef, value);
      return;
    }
  ...
  _spawnMine(x, y) {
    if (this.phase === 99) return;
    const g = this.add.graphics().setDepth(16).setPosition(x, y);
    g.lineStyle(1.5, 0x00ccff, 0.9);
    g.strokeCircle(0, 0, 6);
    g.fillStyle(0x00ccff, 0.5);
    g.fillCircle(0, 0, 3);
    this.tweens.add({ targets: g, alpha: 0.25, duration: 420, yoyo: true, repeat: -1 });
    const body = this.matter.add.circle(x, y, 8, { isStatic: true, label: 'mine', isSensor: true });
    this._mines.push({ x, y, gfx: g, body, blastR: 80, triggered: false });
  }
  ...
  _triggerMine(mine) {
    ...
    this.allDice.forEach(d => {
      ...
      Phaser.Physics.Matter.Matter.Body.setVelocity(d.img.body, {
        x: (dx / dist) * BLAST_SPEED,
        y: (dy / dist) * BLAST_SPEED,
      });
      this._rerollDie(d);
    });
  }
  ...
  _buildPlayerInspector(data, currentIdx) {
    ...
    // Collect upgrade objects for display
    const upgrades = (data.upgradeState?.takenUpgrades ?? [])
      .map(id => UPGRADE_MAP[id]).filter(Boolean);
    const upgradeH = upgrades.length > 0 ? 20 + upgrades.length * 14 + 6 : 0;

    const matH   = hasMaterial ? 16 : 0;
    const runeH  = hasAnyRune  ? 52 : 0;
    const panelW = Math.max(netW + 24, upgrades.length > 0 ? 200 : 0);
    const panelH = netH + 64 + matH + runeH + upgradeH;
    ...
    if (upgrades.length > 0) {
      const secTop = cy - panelH / 2 + 38 + matH + netH + runeH + 10;
      this.inspectorPanel.add(this.add.text(cx, secTop, 'UPGRADES', {
        fontSize: '10px', color: '#334455', letterSpacing: 2,
      }).setOrigin(0.5, 0));
      upgrades.forEach((upg, i) => {
        this.inspectorPanel.add(this.add.text(cx - panelW / 2 + 10, secTop + 16 + i * 14, `● ${upg.name}`, {
          fontSize: '11px', color: upg.color, fontStyle: 'bold',
        }).setOrigin(0, 0));
      });
    }
  }
  ...
  _addToEffectQueue(dieRef) {
    ...
    if (!this._settledValuesThisTurn) this._settledValuesThisTurn = [];
  ...
  update() {
    ...
    // Magnetic upgrade: curve toward nearest bumper while rolling
    if (this.phase === PHASE.PLAYER_ROLL) {
      this.playerDice.forEach(d => {
        if (!d._rolling || !d.img?.active || !d.img.body) return;
        if (!this._hasDieUpgrade(d.data, 'magnetic')) return;
        const ex = this.enemyPos.x, ey = this.enemyPos.y;
        const px = THROW_ORIGIN_X, py = THROW_ORIGIN_Y;
        const dE = Math.hypot(d.img.x - ex, d.img.y - ey);
        const dP = Math.hypot(d.img.x - px, d.img.y - py);
        const tx = dE < dP ? ex : px;
        const ty = dE < dP ? ey : py;
        const dx = tx - d.img.x, dy = ty - d.img.y;
        const len = Math.hypot(dx, dy) || 1;
        Phaser.Physics.Matter.Matter.Body.applyForce(d.img.body, { x: d.img.x, y: d.img.y }, { x: (dx/len)*0.00006, y: (dy/len)*0.00006 });
      });
    }
